...
function AbroadVacationBuildContent() {
    ...
    <Screen withBottomNavSpacing={false}>
      <div className="mx-auto flex max-w-xl flex-col gap-4 pt-6">
        ...
        {awaitingContinueDecision ? (
          ...
          <SwipeCard key={candidate.id} onSwipeLeft={handleUnlike} onSwipeRight={handleLike} disabled={busy}>
      ...
      {showInfo && candidate && <PlaceInfoSheet candidate={candidate} onClose={() => setShowInfo(false)} />}
    </Screen>
  );
}

export default function AbroadVacationBuildPage() {
  return (
    <Suspense
      fallback={
        <Screen>
