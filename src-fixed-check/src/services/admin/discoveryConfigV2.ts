...
export const DISCOVERY_BUCKETS: DiscoveryBucket[] = [
  {
    ...
    categories: [
      ...
      // אפשרות נוספת, בנוסף לכל מה שיש למעלה: מסעדה כשרה. בניגוד לשאר
      // התגיות כאן (שהן cuisine_tags בלבד) - הבחירה בקטגוריה הזו ב-AI
      // Discovery גם מסמנת kosher=true על המקום עצמו (ראו collectionService.ts),
      // כי זה השדה שבאמת נבדק בצורה מחייבת במאגר המועמדים למשתמש שסימן "כשר".
      { key: "kosher", emoji: "✡️", label: "מסעדה כשרה" },
    ],
  },
  {
    key: "attractions",
