...
/**
 * *** מיפוי חדש - "כלל הברזל": Place לא נעלם מה-ADMIN רק כי עדיין אין
 * לו סיווג משני (trip_type_tags/subcategory/tags/cuisine_tags). כל
 * אחד מ-5 סוגי הטיול המבוססי-סקשנים (לא abroad/weekend - להם מנגנון
 * נפרד, destination_editions/רדיוס geo) ממופה לרשימת קטגוריות ראשיות
 * *רלוונטיות* - כשנכנסים לסוג הטיול ברמת "הכל" (לפני בחירת פיל ספציפי),
 * מוצגים *כל* המקומות בקטגוריות האלה, גם ללא סיווג משני - זה הופך את
 * העמוד ל"inbox" לסיווג במקום שער שחוסם תוכן. בחירת פיל ספציפי (למשל
 * "ישראלי") עדיין מסננת מדויק כרגיל - שתי הרמות פועלות זו לצד זו.
 *
 * הבחירה לכל סוג טיול מבוססת על אילו קטגוריות ראשיות באמת מיוצגות
 * בסקשנים שלו (ר' adminDiscoverySections.ts): day_trip פרוס על פני
 * מסעדות/טבע/אטרקציות (עגלות קפה, יקבים, מסלולי טבע, פארקים, אטרקציות,
 * תרבות, היסטוריה); nature_trip ו-restaurants_cafes ו-nightlife כל אחד
 * ממוקד בקטגוריה אחת; romantic_date חוצה מסעדות/טבע/אטרקציות (ארוחה
 * רומנטית, שקיעה ונוף, תרבות והופעה) - לא כולל nightlife/hotels כברירת
 * מחדל כי אלה פחות מרכזיים לרוב הסקשנים שלו.
 */
export const QUICK_CATEGORY_FALLBACK_MAIN_CATEGORIES: Partial<Record<string, PlaceCategoryKey[]>> = {
  day_trip: ["restaurants", "nature", "attractions"],
  nature_trip: ["nature"],
  restaurants_cafes: ["restaurants"],
  romantic_date: ["restaurants", "nature", "attractions"],
  nightlife: ["nightlife"],
};
