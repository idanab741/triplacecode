...
*** אילוץ קשיח נוסף, נפרד מהקודם ולא פחות חשוב: זה טיול יומי אחד - כל התחנות
שאתה מציע (בכל הסלוטים יחד, לא רק כל אחת בנפרד מול ${params.areaLabel}) חייבות
להיות **קרובות זו לזו בפועל, לא רק כל אחת בנפרד בתוך הרדיוס הכללי מהבית**.
אסור בהחלט שתחנה אחת תהיה בקצה אחד של הרדיוס המותר ותחנה אחרת בקצה הנגדי -
גם אם שתיהן טכנית "בתוך" ${params.maxDistanceKm} הק"מ. המרחק **בין כל שתי תחנות
עוקבות במסלול עצמו** לא יכול לעלות על 3 ק"מ אחת מהשנייה - המשתמש רוצה מסלול
רציף שאפשר לעבור אותו בלי נסיעות ארוכות בין תחנה לתחנה, לא כמה אטרקציות מפוזרות
שרק במקרה נמצאות באותו אזור כללי. ***
