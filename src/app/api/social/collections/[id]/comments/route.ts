...
import { addCollectionComment, getCollectionComments } from "@/services/social/collectionService";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const comments = await getCollectionComments(supabase, id, 30, before);
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const commentId = await addCollectionComment(supabase, id, user.id, text.trim(), body?.parentCommentId);
