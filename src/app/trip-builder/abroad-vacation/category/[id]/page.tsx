...
import { WORLDWIDE_VACATION_CATEGORIES, WORLDWIDE_DESTINATION_REGISTRY, CRUISE_LINES } from "@/constants/worldwideVacationCategories";
 ...
 *
 * id="cruise" הוא מקרה מיוחד (תיקון Product מפורש - "קרוזים ושייט
...
export default function AbroadVacationCategoryPage() {
  ...
  const category = WORLDWIDE_VACATION_CATEGORIES.find((c) => c.id === params.id);

  if (!category) {
    return (
      <div className="min-h-screen bg-bg">
        <SimpleAppHeader onBack={() => router.back()} title="קטגוריה" />
        <div className="flex flex-col items-center justify-center gap-3 px-6 py-24 text-center">
          <p className="text-lg font-bold text-ink">הקטגוריה לא נמצאה</p>
          <Link href="/trip-builder/abroad-vacation/discover" className="text-sm text-accent">
            חזרה לחופשה בחו״ל
          </Link>
        </div>
        <MainBottomNav active="home" />
    ...
    <div className="min-h-screen bg-bg pb-36">
      ...
      <div className="grid grid-cols-2 gap-3 px-6 pt-5">
        {category.destinations.map((ref, i) => {
          ...
          const className = "relative block aspect-[3/4] w-full overflow-hidden rounded-card bg-bg-secondary shadow-soft";
          return href ? (
