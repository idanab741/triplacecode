...
// אותה קטגוריה קיימת ("nature_trip") מדף הבית - ר' constants/quickCategories.ts.
const NATURE_CATEGORY = QUICK_CATEGORIES.find((c) => c.id === "nature_trip")!;

const DISCOVERY_LOCATION_STORAGE_KEY = "triplace_nature_trip_discover_location";
...
/**
 * עמוד Discovery של "טיול בטבע" (Audit - "לפי הקטגוריות (אופי הטיול
 * שמופיע בצ'אט)... כמובן לפי החלוקה שהייתה עד כה: בר עליון/HERO/אייקון
 * וטקסט/לוקיישן בצד שמאל/הכי חמים ראשון/ואז כל השאר"): אותו Shell
 * בדיוק כמו day-trip/vacation-il/abroad-vacation/restaurants-cafes.
 * הסקשנים הם 13 "אופי הטיול" מ-NATURE_TYPE_OPTIONS (locales/he/natureTrip.ts,
 * כבר בשימוש בשאלון ה-Trip Builder הקיים) - לא רשימה חדשה. ר' route.ts
 * למיפוי המדויק ל-subcategories אמיתיים (ולשני הפערים הידועים - מעיינות/
 * מפלים, שאין להם תג ייעודי).
 */
export default function NatureTripDiscoverPage() {
  ...
  const [data, setData] = useState<NatureApiResponse | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/discovery/nature-trip?${params.toString()}`)
  ...
  function buildSeeAllHref(sectionId: string): string {
    ...
    if (selectedAddress?.city) params.set("city", selectedAddress.city);
    return `/trip-builder/nature-trip/discover/category?${params.toString()}`;
  }
  ...
  return (
    <div className="min-h-screen bg-bg pb-36">
      ...
      {/* 2. HERO - full-bleed, עם ה-asset הקיים לטיול בטבע. */}
      <div className="relative w-full">
        <Image src="/images/hero-nature-trip.png" alt="" width={800} height={450} priority className="h-auto w-full" />
      </div>
      ...
      <div className="flex items-center justify-between px-6 pt-4">
        <div className="flex items-center gap-2">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full shadow-soft">
            ...
            <img src={NATURE_CATEGORY.imageSrc} alt="" className="h-full w-full object-cover" />
          </span>
          <span className="text-lg font-bold text-ink">{QUICK_CATEGORY_LABELS.nature_trip}</span>
        </div>
