...
interface NightlifeApiResponse {
  hotPlaces: DiscoveryPlace[];
  sections: { id: string; emoji: string; title: string; places: DiscoveryPlace[] }[];
}

// אותה קטגוריה קיימת ("nightlife") מדף הבית - ר' constants/quickCategories.ts.
const NIGHTLIFE_CATEGORY = QUICK_CATEGORIES.find((c) => c.id === "nightlife")!;

const DISCOVERY_LOCATION_STORAGE_KEY = "triplace_nightlife_discover_location";
...
/**
 * עמוד Discovery של "חיי לילה ובילויים" (Audit - "עכשיו אני רוצה חיי
 * לילה ובילויים" - לפי אותה חלוקה: בר עליון/HERO/אייקון וטקסט/לוקיישן
 * בצד שמאל/הכי חמים ראשון/ואז כל השאר): אותו Shell בדיוק כמו day-trip/
 * vacation-il/abroad-vacation/restaurants-cafes/nature-trip/romantic-date.
 * הסקשנים הם 10 "אופי הבילוי" מ-NIGHTLIFE_VENUE_TYPE_OPTIONS
 * (locales/he/nightlife.ts, כבר בשימוש בשאלון ה-Trip Builder הקיים) -
 * לא רשימה חדשה. ר' route.ts למיפוי המדויק ל-tags אמיתיים
 * (PLACE_TYPE_TAGS, לא רק category גס).
 */
export default function NightlifeDiscoverPage() {
  ...
  const [data, setData] = useState<NightlifeApiResponse | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/discovery/nightlife?${params.toString()}`)
  ...
  function buildSeeAllHref(sectionId: string): string {
    ...
    if (selectedAddress?.city) params.set("city", selectedAddress.city);
    return `/trip-builder/nightlife/discover/category?${params.toString()}`;
  }
  ...
  return (
    <div className="min-h-screen bg-bg pb-36">
      ...
      {/* 2. HERO - full-bleed, עם ה-asset הקיים לחיי לילה ובילויים. */}
      <div className="relative w-full">
        <Image src="/images/hero-nightlife.png" alt="" width={800} height={450} priority className="h-auto w-full" />
      </div>

      {/* 3. שורה: אייקון+"חיי לילה ובילויים" בצד ימין, מיקום בצד שמאל. */}
      <div className="flex items-center justify-between px-6 pt-4">
        <div className="flex items-center gap-2">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full shadow-soft">
            ...
            <img src={NIGHTLIFE_CATEGORY.imageSrc} alt="" className="h-full w-full object-cover" />
          </span>
          <span className="text-lg font-bold text-ink">{QUICK_CATEGORY_LABELS.nightlife}</span>
        </div>
