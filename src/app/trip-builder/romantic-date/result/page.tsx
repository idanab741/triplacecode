...
function RomanticDateResultContent() {
  ...
  // אותה בעיה בדיוק שהייתה במסעדות/חיי לילה: fetch יחיד בלי polling
  // "תפס" את ה-session עדיין ב-status="building" (הבנייה בפועל לוקחת
  // עשרות שניות) והציג "לא נבחרו מספיק תחנות" מיידית, בזמן שהבנייה
  // עוד לא הסתיימה בכלל ברקע. תוקן לאותו פתרון - polling עד
  // status==="completed" בפועל, עם מסך טעינה בסגנון משחק בינתיים.
  useEffect(() => {
    if (!sessionId) return;
    let cancelled = false;
    ...
    function fetchOnce() {
      fetch(`/api/trip-builder/sessions?sessionId=${sessionId}`)
        ...
        .catch(() => {
          if (!cancelled) setError("לא הצלחנו לטעון את הדייט");
        });
    }
  ...
  if (session.status !== "completed") {
    return (
      <LoadingGame
        statusText="רגע, בונים לכם את הדייט..."
        steps={[
          "💑 מחפשים מקומות רומנטיים מתאימים",
          "🍽️ בוחרים לפי מה שסימנתם",
