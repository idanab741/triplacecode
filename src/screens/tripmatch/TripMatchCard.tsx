...
/** *** שונה (בקשה מפורשת - "בחלק התחתון של הכרטיסייה יהיו הכפתורים שלנו"):
 *  לפני זה זה היה שטח שמור *מתחת* לכרטיס (הכפתורים רכבו חצי-חצי על
 *  הקצה התחתון שלו). עכשיו הכרטיס תופס 100% מגובה ההורה שלו (אין יותר
 *  "שטח מת" מתחתיו - זה חלק מהבקשה שהכרטיסייה תתארך עד קצה העמוד) והכפתורים
 *  יושבים *לגמרי בתוך* הכרטיס, קרוב לקצה התחתון שלו - הערך הזה הוא המרחק
 *  (px) בין הקצה התחתון של שורת הכפתורים לקצה התחתון של הכרטיס.
 *  page.tsx משתמש באותו מספר בדיוק (מיובא מכאן) למיקום שורת הכפתורים -
 *  כדי שהכל יתיישר לאותו קו. */
  ...
  return (
    ...
    >
      ...
      {/* תוכן תחתון - שם/תיאור/מטא-דאטה/תגיות, עברי-כתב לבן על התמונה.
          *** שונה (בקשה מפורשת - "בחלק התחתון של הכרטיסייה יהיו הכפתורים
          שלנו"): הכפתורים כבר לא רוכבים על הקצה התחתון (חצי בפנים/חצי
          בחוץ) - הם לגמרי בתוך הכרטיס עכשיו, מ-24px עד 122px מהתחתית
          (ZONE + SIZE). ה-padding גדל בהתאם (היה 60px) כדי שהטקסט/התגיות
          תמיד יישארו מעל שורת הכפתורים, לא מתחתיה. */}
      <div
        className="pointer-events-none absolute bottom-0 flex flex-col gap-1.5 px-[18px] pb-[140px] text-white"
