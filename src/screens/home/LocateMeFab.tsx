 ...
 * *מעליו* בדיוק כפי שביקשת. סגנון משני (לבן/outline) - מכוון
 * להיבדל מהכפתור הראשי (+), לא "להתחרות" איתו ויזואלית.
 */
export function LocateMeFab({ onClick }: LocateMeFabProps) {
  return (
    <button
      ...
      className="fixed z-40 flex h-11 w-11 items-center justify-center rounded-full bg-white text-ink shadow-soft transition active:scale-95"
      style={{
        left: "1.25rem",
        // מעל AddPlaceFab בדיוק: bottom שלו (78px מעל ה-nav) + הגובה
        // שלו (56px, h-14) + רווח קטן (10px) ביניהם.
        bottom: "calc(max(env(safe-area-inset-bottom), 22px) + 144px)",
      }}
    >
