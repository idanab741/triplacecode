...
/**
 * *** שונה מהיסוד (בקשה מפורשת - "בהתחלה השורת חיפוש לא תופיע - ורק
 * ברגע שמחליקים למטה בראש הדף (מושכים עם האצבע כלפי מטה, לא גלילה
 * רגילה) היא תופיע, במקום ההחלקה-למטה שהייתה מביאה לטעינת העמוד. נשארת
 * גלויה עד שמתחילים לגלול/להחליק כלפי מעלה לתוך הכרטיסים - כל משיכה
 * כלשהי מיד מופיעה במלואה, לא בהדרגה"):
 *
 ...
 * למרחק המשיכה). היא נשארת גלויה עד שמתחילה גלילה אמיתית של הדף (scrollY
 * זז משמעותית מ-0, כלומר המשתמש בפועל גולל/מחליק כלפי מעלה אל תוך
 * הכרטיסים) - אז היא נסגרת שוב, וחוזרים למצב "מוסתר כברירת מחדל".
 *
 ...
 * transition קלה) - *לא* טריק margin-bottom+transform מפצה (שהיה בגרסה
 * קודמת): הטריק ההוא הניח שההתכווצות קורית *רק* תוך כדי גלילה בפועל (אז
 * ה-margin המתווסף "נבלע" ע"י אותה כמות גלילה בדיוק) - אבל כאן ההסתרה
 * יכולה לקרות בלי שום גלילה (מיד בטעינה), אז margin קבוע כזה היה נשאר
 * רווח ריק אמיתי במקום. עכשיו: מוסתר = פשוט קטן יותר במקום (בלי טריקים).
...
export function CollapsibleTopBar({
  ...
  useEffect(() => {
    ...
    function apply() {
      ...
      // *** תוקן (Bug מפורש - "רווח גדול בין הבר לקטגוריות" + "הכרטיסייה
      // בורחת מהעמוד"): לפני זה ה"הסתרה" הייתה משאירה marginBottom קבוע
      // (=naturalHeight) על הבר, ומזיזה את *תוכן* השורה למעלה עם transform
      // - טריק שנועד לגלילה *הדרגתית* (כל פיקסל גלילה מכווץ את ה-clip
      // וגם מגדיל בדיוק אותו marginBottom, כך שהסכום נשאר קבוע וזה נראה
      // כאילו השורה "נגללת מתחת" לבר הנדבק). עכשיו ההסתרה היא מצב בינארי
      // שיכול לקרות *בלי שום גלילה בכלל* (מיד בטעינת העמוד) - ואז אין שום
      // "גלילה מפצה" שסופגת את ה-marginBottom הזה, והוא פשוט נשאר רווח
      // ריק אמיתי מתחת לבר (ומכיוון ש-margin לא מתכווץ ע"י flex-shrink,
      // הוא גם דוחף את הגובה הכולל מעבר לתיבה הקבועה של הכרטיס ב-
      // home/page.tsx - זה מה שגרם ל"בריחה"). התיקון: בלי marginBottom
      // ובלי transform בכלל - רק clip.height="0px" (accordion רגיל, לא
      // טריק גלילה) - כשמוסתר, הבר פשוט קטן יותר במקום, ותוכן שמתחתיו
      // עולה למקומו הרגיל, בלי רווח שיורי.
