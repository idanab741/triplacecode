...
export function HomeQuickCategories({ selected, onToggle, leading }: HomeQuickCategoriesProps) {
  ...
  return (
    <div
      ...
      className="flex w-full min-w-0 max-w-full gap-2 overflow-x-auto ps-6 pb-1 pt-1"
      // *** הוסר (בקשה מפורשת - "אי אפשר להחליק ימינה ושמאלה בשורה של
      // סוגי הטיול", אחרי שגם ההחלקה בשאר האפליקציה - למשל 'כל מה
      // שחם', HomeHotRow - עובדת בלי שום touch-action מפורש): ה-
      // touch-action:pan-x שהיה כאן נוסף בזמנו כדי לפצות על רשת ביטחון
      // גורפת (html { overflow-x:hidden }) שכבר לא קיימת יותר (ר'
      // globals.css) - אבל בפועל, ההצהרה המפורשת הזו עצמה (ולא
      // ה-overflow-x-auto הרגיל) היא מה ש"תפס" בסביבה הנוכחית ומנע את
      // תחילת ההחלקה: pan-x דוחה כל מגע עם רכיב אנכי ולו הקל ביותר
      // (כמעט כל נגיעה אמיתית באצבע), במקום לתת לדפדפן לזהות את
      // הכיוון בעצמו. הוסר, ונשאר רק overflow-x-auto - בדיוק כמו
      // HomeHotRow, שעובד.
