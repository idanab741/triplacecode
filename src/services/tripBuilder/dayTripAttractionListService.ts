...
*** חובה: כל מקום אמיתי וקיים בפועל, בטווח נסיעה סביר (עד ${params.maxDistanceKm} ק"מ)
מ${params.areaLabel}. **חשוב: אל תעדיף באופן שיטתי את המקום הכי קרוב** - המשתמש אישר
טווח נסיעה עד ${params.maxDistanceKm} ק"מ, ופיזור אמיתי על פני הטווח (לא רק ליד הבית)
נותן מגוון טוב יותר. יעד מומלץ למרחק כולל של המסלול היום: בסביבות ${params.targetDistanceKm}
ק"מ מנקודת המוצא (לא חובה מדויקת, אבל אל תתכנס תמיד לכיוון 0). ***
