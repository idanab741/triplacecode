...
export async function generateDayTripPlaces(params: GenerateDayTripParams): Promise<ResolvedDayTripPlace[]> {
  ...
  const slotsBreakdown = params.slots
    .map((s, i) => `${i + 1}. slotId: "${s.slotId}", role: ${s.role}, category: ${s.category} (זו תחנה מספר ${i + 1} מתוך ${params.slots.length} ברצף היום)`)
    .join("\n");
...
מ${params.areaLabel}. **אל תבחר את המקום הכי מובן מאליו והכי מוכר בקטגוריה** (כמו
