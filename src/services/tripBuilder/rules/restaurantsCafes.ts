...
המטרה אינה למצוא מקום לאכול, אלא להמליץ על המקום המתאים ביותר בדיוק לרגע הזה, ואז
לבנות סביבו בילוי קטן ומשלים (לא רק ארוחה בודדת).

הקטגוריות משתנות לפי שעת היציאה:
- בוקר: בית קפה → בראנץ' → קינוח → אולי טיול קצר.
- צהריים: מסעדה → גלידריה → אולי תצפית.
- ערב: מסעדה → בר → קינוח.

אם המשתמש בחר "בית קפה" (לא מסעדה) - יוצג אחריו מאפייה/קינוחים, ואז אולי מקום לטיול קצר.
אם המשתמש בחר "מסעדה" - יוצג אחריו בר, ואז קינוח, ואז אולי תצפית.
...
- כל מקום חייב להיות פתוח בזמן ההגעה המשוער.
- כל מקום חייב לעמוד בתקציב שנבחר.

--- אילוצים טכניים ליישום בקוד ---
- קבע role לכל תחנה: attraction / food / coffee_dessert / viewpoint בלבד.
...
המלל החופשי הוא מרכז ההחלטה (כ-60-70%) - נתח אווירה רצויה, מגבלות, בקשות מיוחדות
(נוף, ישיבה בחוץ, חניה, שקט וכו'). תחומי העניין הקולינריים שסומנו הם משני (כ-20-30%).
פרופיל/היסטוריית העדפות משלימים (כ-10%).
