      ...
      {HOME_QUICK_CATEGORIES.map((category) => {
        ...
        return (
          <button
            ...
            // *** אותו רוחב קבוע כמו קודם (w-[76px]) - ר' הערה מקורית:
            // מבטיח מרווח אחיד בין העיגולים, לא תלוי באורך התווית.
            className="flex w-[76px] shrink-0 flex-col items-center gap-1.5"
          >
            <span
              className="flex h-15 w-15 items-center justify-center overflow-hidden rounded-full shadow-soft transition-shadow"
              style={
                isSelected
                  ? { boxShadow: `0 0 0 3px var(${category.colorVar}), 0 4px 10px rgba(16,24,40,0.18)` }
                  : undefined
              }
            >
