...
export default function AdminPlacesPage() {
  ...
  // "מסעדה כשרה" - אפשרות נוספת לצפייה/סינון של מאגר המקומות שסומנו kosher=true.
  // זה בדיוק המאגר שממנו fetchCandidatePool שולף באופן מחייב (kosher.eq.true,
  // בלי null) למשתמש קצה שסימן בהעדפות שלו שהוא רוצה כשר - ר' candidatePoolService.ts.
  ...
  const filteredPlaces = places.filter((p) => {
    ...
    if (kosherOnly && p.kosher !== true) return false;
    ...
    <div dir="rtl" className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        <button
          ...
          title="מציג רק מקומות שסומנו kosher=כן - זה בדיוק המאגר שנאכף באופן מחייב למשתמשים שסימנו 'כשר' בהעדפות שלהם"
          className="rounded-[var(--admin-radius-sm)] border px-3.5 py-2 text-[13.5px] font-medium transition"
          style={{
            borderColor: kosherOnly ? "var(--admin-accent)" : "var(--admin-border)",
            background: kosherOnly ? "var(--admin-accent-soft)" : "var(--admin-bg-surface)",
            color: kosherOnly ? "var(--admin-accent)" : "var(--admin-ink)",
          }}
        >
          ✡️ מסעדה כשרה {kosherOnly && `(${places.filter((p) => p.kosher === true).length})`}
        </button>
